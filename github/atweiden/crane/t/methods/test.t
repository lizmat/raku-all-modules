...
subtest({
    ...
    ok(
        Crane.test(%data, :path('legumes', 0, 'name'), :value("pinto beans")),
        'Is expected value'
    );
    ok(
        Crane.test(%data, :path('legumes', 0, 'instock'), :value(4)),
        'Is expected value'
    );
    ...
    ok(
        Crane.test(%data, :path('legumes', 3, 'instock'), :value(8)),
        'Is expected value'
    );
    throws-like(
        { Crane.test(%data, :path(qw<a b c>), :value(1)) },
