...
subtest({
    ...
    is-deeply(
        %h,
        {:level-one({:level-two({:level-three(3)})})},
    ...
    is-deeply(
        %data-new<legumes>[0],
        {:name<carrots>, :unit<lbs>, :instock(3)},
        'Is expected value'
    );
...
subtest({
    ...
    my %h = :q({ :bar(2) });
    throws-like(
        { Crane.add(%h, :path(qw<a b>), :value(7)) },
    ...
    throws-like(
        { Crane.add($list, :path(*-0,), :value<three>) },
        X::Crane::Add::RO,
    ...
    throws-like(
        # invalid: Int < 0
        { Crane.add(@a, :path(-1,), :value(True)) },
        X::Crane::PositionalIndexInvalid,
        'Positional index invalid'
    );
    throws-like(
        # invalid: Str
        { Crane.add(@a, :path('0',), :value(True)) },
        X::Crane::PositionalIndexInvalid,
        'Positional index invalid'
    );
    ...
    # path not found
    throws-like(
        { Crane.add(%doc, :path('a', 0, 'b'), :value<barf>) },
        X::Crane::AddPathNotFound,
        'Add operation fails when path not found'
    );
    ...
    throws-like(
        { Crane.add(%doc, :path('a', *-2), :value<eight>) },
    ...
    throws-like(
        { Crane.add(%doc, :path('a', *-1), :value<eight>) },
        X::Crane::AddPathOutOfRange,
        :message(
            /'add operation failed, Positional index out of range.'/
        ),
    ...
    throws-like(
        { Crane.add(%doc, :path('a', 8), :value<eight>) },
        X::Crane::AddPathOutOfRange,
        :message(
            /'add operation failed, creating sparse Positional not allowed.'/
        ),
