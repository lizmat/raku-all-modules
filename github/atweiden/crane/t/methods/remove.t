...
subtest({
    ...
    is-deeply(@h, ['she','want','more',['more',['more']]], 'Is expected value');
    is-deeply(
        @g,
...
subtest({
    my %h = :a<alpha>,:b<bravo>,:c<charlie>;
    throws-like(
        { Crane.remove(%h, :path('d',)) },
        X::Crane::RemovePathNotFound,
        'Remove operation fails when path not found'
    );
    my %i = :a(:b(:c(True)));
    throws-like(
        { Crane.remove(%i, :path(qw<a b c>)) },
        X::Crane::Remove::RO,
        'Remove operation fails when target is immutable'
    );
    my @a = qw<zero one two>;
    throws-like(
        { Crane.remove(@a, :path(*-0,)) },
        X::Crane::RemovePathNotFound,
        'Remove operation fails when path not found'
    );
    my $list = (qw<zero one two>);
    throws-like(
        { Crane.remove($list, :path(0,)) },
        X::Crane::Remove::RO,
        'Remove operation fails when target is immutable'
    );
    throws-like(
        { Crane.remove($list, :path(9,)) },
        X::Crane::RemovePathNotFound,
        'Remove operation fails when path not found'
    );
    throws-like(
        { Crane.remove($list, :path('d',)) },
