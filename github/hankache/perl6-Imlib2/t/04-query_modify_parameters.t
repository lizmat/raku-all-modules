...
lives-ok { $im.image_set_has_alpha(True); }, 'image_set_has_alpha - set has alpha to True';
is $im.image_has_alpha(), True, 'image_has_alpha returns True';
lives-ok { $im.image_set_has_alpha(False); }, 'image_set_has_alpha - set has alpha to False';
is $im.image_has_alpha(), False, 'image_has_alpha returns False';

lives-ok { $im.image_set_changes_on_disk(); }, 'image_set_changes_on_disk';

my $structure = Imlib2::Border.new();
isa-ok $structure, Imlib2::Border;
...
#border functions not working currently.
#my $border = $structure.init();
#ok $border, 'returns a pointer which contains the values ​​of the structure';

#lives-ok { $im.image_set_border($border); }, 'image_set_border';
#lives-ok { $im.image_get_border($border); }, 'image_get_border';

#lives-ok { $structure.get($border); }, 'get the values of the border stored in structure';
...
lives-ok { $im.image_set_irrelevant(format => True); }, 'image_set_irrelevant_format is set to True';
lives-ok { $im.image_set_irrelevant(format => False); }, 'image_set_irrelevant_format is set to False';
lives-ok { $im.image_set_irrelevant(border => True); }, 'image_set_irrelevant_border is set to True';
lives-ok { $im.image_set_irrelevant(border => False); }, 'image_set_irrelevant_border is set to False';
lives-ok { $im.image_set_irrelevant(alpha => True); }, 'image_set_irrelevant_alpha is set to True';
lives-ok { $im.image_set_irrelevant(alpha => False); }, 'image_set_irrelevant_alpha is set to False';
