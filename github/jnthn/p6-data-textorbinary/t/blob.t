...
ok is-text("Wow!\nIt's text and stuff!\n".encode('ascii')),
    'Basic text with some UNIX newlines is considered text';
ok is-text("Wow!\r\nIt's text and stuff!\r\n".encode('ascii')),
    'Basic text with some Windows newlines is considered text';
nok is-text("Seems OK but oh a \0 NULL!".encode('ascii')),
    'Basic text but with a NULL byte is always binary';
nok is-text("Seems OK but oh a \r isolated CR!".encode('ascii')),
    'Basic text but with a CR byte is always binary';
ok is-text("Strč prst skrz krk! Vánoční stromeček".encode('utf-8')),
    'Text with non-ASCII characters encoded in UTF-8 comes out as text';
nok is-text(Buf.new(0x01, 0x02, 0x65, 0x66, 0x31)),
    'Binary data with the odd bit of text comes out binary';

ok is-text(q:to/C HEADER/.encode('ascii')), 'Sample C header file comes out as text';
