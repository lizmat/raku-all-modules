...
class CompUnit::Repository::Tar does CompUnit::Repository {
    ...
    submethod TWEAK(:$!name = 'tar') {
        CompUnit::RepositoryRegistry.register-name($!name, self);
    }

    # for testing purposes only
    method test-dist($basename) {
        %?RESOURCES{"test-dists/$basename"}.IO.absolute
    }
    ...
    method installed(--> Seq) {
        $dists-lock.protect: {
            @!dists = @!dists.elems ?? @!dists !! do {
                my $tar-dist  = Distribution::Common::Tar.new($!prefix.IO);
                my $cur-dist = CompUnit::Repository::Distribution.new($tar-dist);
                $tar-dist does CURID($cur-dist.id);
                $tar-dist;
            }
            return @!dists.Seq;
        }
    ...
    method short-id(--> Str:D)     { 'tar' }
    method id(--> Str:D)           { $!prefix ~ join '#', self.installed.map(*.id) }
    method can-install(--> Bool:D) { False }
    method path-spec(--> Str:D)    { "CompUnit::Repository::Tar#{$!prefix}" }
