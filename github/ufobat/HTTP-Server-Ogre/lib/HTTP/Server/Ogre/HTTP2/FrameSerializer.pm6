...
class HTTP::Server::Ogre::HTTP2::FrameSerializer {


    method serialize-frames(Supply:D $in, $connection-state) {
        supply {
            ...
            sub send-splitted($frame) {
                my $is-header = $frame ~~ HTTP::Server::Ogre::HTTP2::Frame::Headers;
            ...
            with $connection-state.ping {
                whenever $connection-state.ping.Supply {
                    my $ack = HTTP::Server::Ogre::HTTP2::Frame::Ping.new(
            ...
            with $connection-state.settings {
                whenever $connection-state.settings.Supply {
                    when Bool { # Preface case
                        ...
                        my $set = HTTP::Server::Ogre::HTTP2::Frame::Settings.new(
                            flags             => 0,
                            stream-identifier => 0,
                            settings          => (1 => 4096,  2 => 0,
                                                  3 => 100,   4 => 65535,
                    ...
                    default {
                        ...
                        my $ack = HTTP::Server::Ogre::HTTP2::Frame::Settings.new(
            ...
            whenever $in -> HTTP::Server::Ogre::HTTP2::Frame $frame {
                given $frame {
    ...
    method !serializer(Buf $buf, HTTP::Server::Ogre::HTTP2::Frame $_) {
        when HTTP::Server::Ogre::HTTP2::Frame::Data {
        ...
        when HTTP::Server::Ogre::HTTP2::Frame::Settings {
        ...
        when HTTP::Server::Ogre::HTTP2::Frame::PushPromise {
            $buf.append: .padding-length if .padded;
            my $num = .promised-sid;
            for 24, 16...0 {
                $buf.append: (($num +> $_) +& 0xFF);
        ...
        when HTTP::Server::Ogre::HTTP2::Frame::Ping {
            if .payload.elems < 8 {
    ...
    method !form-header(HTTP::Server::Ogre::HTTP2::Frame $_) {
        my $buf = Buf.new;
        my $i = 0;
        ...
        die PROTOCOL_ERROR if $num != 0 && $_ ~~ HTTP::Server::Ogre::HTTP2::Frame::Settings;
        die PROTOCOL_ERROR if $num != 0 && $_ ~~ HTTP::Server::Ogre::HTTP2::Frame::Ping;
        die PROTOCOL_ERROR if $num != 0 && $_ ~~ HTTP::Server::Ogre::HTTP2::Frame::GoAway;
        for 24, 16...0 {
            $buf[$i] = ($num +> $_) +& 0xFF; $i++;
        }
        $buf;
