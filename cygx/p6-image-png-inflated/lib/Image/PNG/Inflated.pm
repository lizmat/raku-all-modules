...
sub crc32(blob8 $bytes) {
    my uint32 $crc = 0xFFFFFFFF;
    loop (my int $i = 0; $i < $bytes.elems; ++$i) {
...
sub adler32(blob8 $bytes) {
    my uint32 $a = 1;
    my uint32 $b = 0;

    loop (my int $i = 0; $i < $bytes.elems; ++$i) {
        $a = ($a + $bytes[$i]) mod 65521;
        $b = ($b + $a) mod 65521;
    }
...
sub deflate(blob8 $bytes) {
    gather {
        take ZLIBHEADER;

        my int $pos = 0;
        my int $remaining = $bytes.elems;
        while $remaining > 0xFFFF {
            my $chunk := $bytes.subbuf($pos, 0xFFFF);
            take blob8.new(0, 0xFF, 0xFF, 0, 0),
                $chunk;
        ...
        my uint8 $hi = $remaining +> 8;
        my uint8 $lo = $remaining +& 0xFF;
        my $chunk := $bytes.subbuf($pos);
        take blob8.new(1, $lo, $hi, +^$lo +& 0xFF, +^$hi +& 0xFF),
            $chunk,
            be32(adler32($bytes));
    }
}
