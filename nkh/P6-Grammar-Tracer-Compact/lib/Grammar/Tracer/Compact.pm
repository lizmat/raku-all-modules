...
my class TracedGrammarHOW is Metamodel::GrammarHOW {
    ...
    method find_method($obj, $name) {
        ...
        return -> $c, |args {
            ...
            @traces = () if $level == 0 ; # reset between grammar matches

            @traces.push: 'this' ;
            my $this := @traces[*-1] ; # @traces has elements added by sub matches, remember where to update
	    ...
	    my $indentation = '   ' x $level ;
            ...
            if $result.MATCH {
                ...
                $this = $indentation ~ colored($name, 'bold green') ~ RESET( ) ~ ' '
                                   #~ ( $result.MATCH.Str.substr(0, $width).perl // '' )
                                   ~ ( $result.MATCH.Str.perl // '' )
            }
            else{
                $this = $indentation ~ colored($name, 'bold red') ~ RESET()
            }
