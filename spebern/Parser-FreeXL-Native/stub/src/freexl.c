...
char *
p6_freexl_get_worksheet_name (const void *xls_handle,
			   unsigned short worksheet_index, int *result)
{
/*
   attempting to retrieve some Worksheet name [by index]
   avoid double pointer of the original function, makes things easier for native call
*/
    unsigned int count = 0;
    biff_sheet *worksheet;
    biff_workbook *workbook = (biff_workbook *) xls_handle;
    ...
    while (worksheet)
	  ...
	  if (count == worksheet_index)
	    {
		worksheet_name = worksheet->utf8_name;
                *result = FREEXL_OK;
    ...
    *result = FREEXL_BIFF_ILLEGAL_SHEET_INDEX;
    return NULL;
}
