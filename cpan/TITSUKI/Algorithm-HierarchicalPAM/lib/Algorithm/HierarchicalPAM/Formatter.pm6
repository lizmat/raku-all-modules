...
method from-plain(@documents is raw, &tokenizer = { .words }) {
    my %word2type;
    my $destringed-documents = CArray[Algorithm::HierarchicalPAM::Document].allocate: +@documents;
    for ^@documents -> $doc-index {
        ...
        $destringed-documents[$doc-index] = Algorithm::HierarchicalPAM::Document.new(:length(+@tokenized), :@words);
    }
...
method from-libsvm(@documents) {
    my CArray $documents = CArray[Algorithm::HierarchicalPAM::Document].allocate: +@documents;
    ...
    for @documents {
        ...
        $documents[$idx++] = Algorithm::HierarchicalPAM::Document.new(:length(+@words), :@words);
    }
    $documents;
}
