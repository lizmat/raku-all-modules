...
class PDF::Annot
    ...
    has PDF::DAO::TextString $.NM is entry(:alias<name>);       #| (Optional; PDF 1.4) The annotation name, a text string uniquely identifying it among all the annotations on its page.
...
##						     :ReadOnly(7) :Locked(8) :ToggleNoView(9) :LockedContents(10) »;
    ...
    has PDF::DAO::Name $.AS is entry(:alias<appearance-state>); #| (Required if the appearance dictionary AP contains one or more subdictionaries; PDF 1.2) The annotation’s appearance state, which selects the applicable appearance stream from an appearance subdictionary
