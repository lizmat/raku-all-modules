...
use StrictClass;
unit class Net::BGP::Path-Attribute::Next-Hop:ver<0.1.1>:auth<cpan:JMASLAK>
...
# Next-Hop Types
method implemented-path-attribute-code(-->Int) { 3 }
method implemented-path-attribute-name(-->Str) { "Next-Hop" }

method path-attribute-name(-->Str:D) { "Next-Hop" }
...
method from-raw(buf8:D $raw where $raw.bytes == 7, Bool:D :$asn32) {
    if   $raw[0] +& 0x80 { die("Optional flag not valid on Next-Hop attribute") }
    if ! $raw[0] +& 0x40 { die("Transitive flag must be set on Next-Hop attribute") }
...
method from-hash(%params is copy, Bool:D :$asn32)  {
    ...
    my $ip = ipv4-to-buf8( %params<next-hop> );
...
INIT { Net::BGP::Path-Attribute.register(Net::BGP::Path-Attribute::Next-Hop) }
...
=head1 SYNOPSIS
  ...
  my $cap = Net::BGP::Path-Attribute::Next-Hop.from-raw( $raw );
  # or …
  my $cap = Net::BGP::Path-Attribute::Next-Hop.from-hash(
    %{ next-hop => '10.2.3.4' }
  );
