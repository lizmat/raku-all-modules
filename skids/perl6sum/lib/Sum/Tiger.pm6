=NAME Sum::Tiger - implementation of Tiger digest family for Sum::
...
=begin DESCRIPTION
    ...
    calculate hashes according to the Tiger algorithm.  Different revisions
    of the algorithm may be selected.

    Tiger sums can be computationally intense.  They also require a small
...
=head2 role Sum::Tiger1 does Sum::MDPad
    ...
    obseleted Tiger-1 version uses a slightly different block padding, and
    due to the history of the specifications, is not well-defined for
    byte-unaligned messages (see NOTES.)
...
role Sum::Tiger {
    ...
    has @!x;     # "Parsed" message gets bound here.
...
0xcd56d9430ea8280e, 0xc12591d7535f5065, 0xc83223f1720aef96, 0xc3a0396f7363a51f);
    ...
    method key_schedule () {
        ...
        # This will get prettier when we get a uint64
        my (Int $x0, Int $x1, Int $x2, Int $x3,
            Int $x4, Int $x5, Int $x6, Int $x7) = @!x;

        $x0 = $x0 - ($x7 +^ 0x1A5A5A5A5A5A5A5A5);
    ...
    method pass ($a is rw, $b is rw, $c is rw, int $mul) {
        for @!x {
            $c = $c +^ $_;
            my ($ta0, $ta1, $ta2, $ta3) =
                @t1[($c      ) +& 255], @t2[($c +> 16) +& 255],
                @t3[($c +> 32) +& 255], @t4[($c +> 48) +& 255];
            my ($tb0, $tb1, $tb2, $tb3) =
                @t4[($c +>  8) +& 255], @t3[($c +> 24) +& 255],
                @t2[($c +> 40) +& 255], @t1[($c +> 56) +& 255];
            $a = $a - ($ta0 +^ $ta1 +^ $ta2 +^ $ta3);
            $b = $b + ($tb0 +^ $tb1 +^ $tb2 +^ $tb3);
...
=begin NOTES
    ...
    On the subject of Tiger version 1 padding, the specification's original
    reference code implied a left-justified last byte.  Then the reference
    implementation was changed to pad in a way that would be consistent with
    a right-justified last byte.  However the reference implementation did
    not cover the case of messages that do not pack evenly into bytes.

    Test vectors, including vectors for messages that do not pack evenly
    into bytes, were then posted which are correct for right-justification
    as implemented in this module.

    The Tiger2 "specification" was then issued by the authors simply
    stating a new padding method "is the same as in MD5/SHA", which implies
    left-justification, thus rolling back the original correction.
...
=item "Tiger: A Fast New Hash Function" (Anderson, Biham) L<http://www.cs.technion.ac.il/~biham/Reports/Tiger/tiger/tiger.html>
