...
ok $parser.stats<xml_objects> eq 5 + 5 + 2, "There must be 12 xml objects total";

$xmldoc = $parser.parse($html, :nowhitespace(True));

ok $parser.stats<xml_objects> eq 5 + 2, "Without whitespace only 7 xml objects";


$xmldoc = $parser.parse($html, :TAG<p>, :nowhitespace(True));

ok $xmldoc.root[0].name eq "p", "We should have <p>";
ok $parser.stats<xml_objects> eq 3, "Only a <p> and <html> make for 3 xml objects";
